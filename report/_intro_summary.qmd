...
gt_summary_table_source <- report_data[[cip_data_col]] |>
  purrr::list_rbind() |>
...
gt_summary_table_agency <- report_data[[cip_data_col]] |>
  purrr::list_rbind() |>
...
size <- 1.8
plot_location_data(report_data, size = size, stroke = size / 8)
```
...
```{r}
if (params$cip_type == "recommendation") {
  summary_amt <- fy_from_gt(gt_summary_table_source)
  recommendation_org_short <- "Board of Finance"
  recommendation_org <- paste0("Baltimore City ", recommendation_org_short)
  recommendation_date <- "March 2024"

  summary_text <- glue("The {recommendation_org_short} is recommending {summary_amt} in funding over 6 years towards the requested projects. This report reflects the recommendations of the {recommendation_org} as of {recommendation_date}.")
} else {
